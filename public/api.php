...
$pdo->exec("CREATE TABLE IF NOT EXISTS trade_licenses (
    ...
    dob VARCHAR(50), gender VARCHAR(50), spouse_name VARCHAR(255), mobile VARCHAR(50), owner_address TEXT, owner_email VARCHAR(255), tin_no VARCHAR(100), bin_no VARCHAR(100), category VARCHAR(255),
    ...
    comm_tax DECIMAL(10,2) DEFAULT 0, sign_tax DECIMAL(10,2) DEFAULT 0, discount_amount DECIMAL(10,2) DEFAULT 0, discount_reason TEXT, total_fee DECIMAL(10,2) DEFAULT 230,
    is_renewal TINYINT(1) DEFAULT 0, original_license_no VARCHAR(100), photo LONGTEXT,
    status VARCHAR(50) DEFAULT 'Pending', apply_date VARCHAR(50), signatory_role VARCHAR(100) DEFAULT 'চেয়ারম্যান',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
)");

foreach (['gender VARCHAR(50)', 'spouse_name VARCHAR(255)', 'owner_email VARCHAR(255)', 'tin_no VARCHAR(100)', 'bin_no VARCHAR(100)', 'biz_permanent_address TEXT', 'biz_present_address TEXT', 'employee_count VARCHAR(50)', 'signboard_size VARCHAR(100)'] as $tradeColumn) {
    try { $pdo->exec("ALTER TABLE trade_licenses ADD COLUMN {$tradeColumn}"); } catch (Exception $e) {}
}
foreach (['discount_amount DECIMAL(10,2) DEFAULT 0', 'discount_reason TEXT'] as $tradeColumn) {
    try { $pdo->exec("ALTER TABLE trade_licenses ADD COLUMN {$tradeColumn}"); } catch (Exception $e) {}
}
...
if ($action === 'submitTradeLicenseApplication' || $action === 'submitTradeRenewalApplication') {
    ...
    $stmt->execute([
        ...
        200, 30, $data['commTax'] ?? 0, $data['signTax'] ?? 0, 0, null, 200 + 30 + (float)($data['commTax'] ?? 0) + (float)($data['signTax'] ?? 0), $isRenewal ? 1 : 0, $isRenewal ? $licNo : null, $data['photo'] ?? '',
...
if ($action === 'updateTradeLicenseData') {
    $appId = trim($data['appId'] ?? '');
    $commTax = max(0, (float)($data['commTax'] ?? 0));
    $signTax = max(0, (float)($data['signTax'] ?? 0));
    $discountAmount = max(0, (float)($data['discountAmount'] ?? 0));
    $grossFee = 200 + 30 + $commTax + $signTax;
    if ($discountAmount > $grossFee) $discountAmount = $grossFee;
    $discountReason = trim($data['discountReason'] ?? '');
    if ($discountAmount > 0 && $discountReason === '') {
        echo json_encode(['success' => false, 'error' => 'ডিসকাউন্ট দেওয়ার কারণ লিখুন।']);
        exit;
    }
    $totalFee = $grossFee - $discountAmount;
    $stmt = $pdo->prepare("UPDATE trade_licenses SET org_name=?, owner_name=?, father_name=?, mother_name=?, nid=?, dob=?, gender=?, spouse_name=?, mobile=?, owner_email=?, tin_no=?, bin_no=?, category=?, fiscal_year=?, owner_address=?, biz_address=?, biz_permanent_address=?, biz_present_address=?, biz_start_date=?, biz_details=?, capital=?, employee_count=?, signboard_size=?, photo=?, comm_tax=?, sign_tax=?, discount_amount=?, discount_reason=?, total_fee=? WHERE app_id=?");
    $stmt->execute([
        $data['orgName'] ?? '', $data['ownerName'] ?? '', $data['fatherName'] ?? '', $data['motherName'] ?? '',
        $data['nid'] ?? '', $data['dob'] ?? '', $data['gender'] ?? 'পুরুষ', $data['spouseName'] ?? '', $data['mobile'] ?? '', $data['ownerEmail'] ?? '', $data['tinNo'] ?? '', $data['binNo'] ?? '', $data['category'] ?? '', $data['fiscalYear'] ?? '',
        $data['ownerAddress'] ?? '', $data['bizAddress'] ?? '', $data['bizPermanentAddress'] ?? '', $data['bizPresentAddress'] ?? '', $data['bizStartDate'] ?? '', $data['bizDetails'] ?? '',
        $data['capital'] ?? '', $data['employeeCount'] ?? '', $data['signboardSize'] ?? '', $data['photo'] ?? '', $commTax, $signTax, $discountAmount, $discountReason,
        $totalFee, $appId
    ]);
    echo json_encode(['success' => true]);
    exit;
